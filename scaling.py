"""Billion-scale, on the back of an envelope — from measured constants.

pipeline: measured million-row numbers ──► [scaling] ──► a defensible billion

scale.py MEASURED a million rows on one laptop. This file does the honest
arithmetic that turns that measurement into a billion, and prints the budget
a real system would be built to. Nothing here is simulated; it is pure
arithmetic over three things the rest of the repo already established:
  ...
  3. SHARDING  a billion is 1,000 shards of the million scale.py already
...
Run me:  python3 scaling.py              # the billion-row budget, printed
...
# bytes per vector, per encoding — the ladder quantize.py and pq.py climb
ENCODINGS = {
...
def main():
    ...
    ap.add_argument("--n", type=float, default=1e9, help="corpus size (default a billion)")
    ...
    print(f"BILLION-SCALE BUDGET for N = {n:,} vectors, {DIM}-d\n")

    print("1. MEMORY — the whole index, per encoding:")
    for enc in ENCODINGS:
        print(f"   {enc:<9} {_human_bytes(memory(n, enc)):>12}"
              f"   ({ENCODINGS[enc]:>4} bytes/vector)")
    print(f"   → PQ-64 is what ships: {_human_bytes(memory(n, 'PQ-64'))} for a billion,"
    ...
    print("3. SHARDING — a billion is k copies of a measured million:")
    ...
    print("5. THE AGENT LAYER, on top, for free:")
